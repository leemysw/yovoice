...
test('角色保存、编辑取消、另存为和应用保留正文', async ({ page }) => {
  await page.goto('/');
  ...
  await dialog.getByLabel('角色名称').fill('不应保存');
  ...
  await expect(dialog.getByRole('button', { name: '保存角色', exact: true })).toBeInViewport();
...
test('角色搜索、空状态和表单校验反馈', async ({ page }, testInfo) => {
  await page.goto('/');
  await page.getByTestId('nav-characters').click();
  await expect(page.getByRole('button', { name: '保存为角色', exact: true })).toHaveCount(1);
  ...
  await expect(dialog.getByRole('button', { name: '保存角色', exact: true })).toBeDisabled();
  await dialog.getByLabel('角色名称').fill('温柔旁白');
  await dialog.getByLabel('试听台词').fill('');
  await expect(dialog.getByRole('button', { name: '生成试听', exact: true })).toBeDisabled();
  await expect(dialog.getByRole('button', { name: '保存角色', exact: true })).toBeEnabled();
  await dialog.getByRole('button', { name: '保存角色', exact: true }).click();
  await expect(page.getByText('角色已保存，可在角色库中查看和使用。')).toBeVisible();
