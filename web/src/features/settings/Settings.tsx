...
export function Settings({ state, catalog, draft, onModel, run }: { state: State; catalog: ModelPackage[]; draft: Draft; onModel: (id: string) => void; run: (task: () => Promise<unknown>) => void }) {
  ...
  return <VStack className="settings-page" gap={6}>
    ...
    {tab === 'models' ? <VStack gap={5} id="models-panel" role="tabpanel" aria-label="模型">
      ...
      <HStack className="runtime-heading" gap={3} vAlign="center"><Cpu size={22} strokeWidth={1.5} /><VStack gap={1}><HStack gap={3} vAlign="center"><h2>audio.cpp</h2><small>v0.7.4</small></HStack><small>本地语音推理</small></VStack></HStack>
      <VStack gap={0}>
