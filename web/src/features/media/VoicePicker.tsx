...
export function VoicePicker({ voices, onClose, onSelect, adding = false }: { adding?: boolean; voices: Voice[]; onClose: () => void; onSelect: (v: Voice) => void }) {
  ...
  return <Dialog className="voice-picker" isOpen onOpenChange={open => { if (!open && !recording && !busy) onClose(); }} width={560} purpose="form" padding={6}>
    <VStack gap={5}>
      <HStack hAlign="between" vAlign="center"><VStack gap={2}><h2>{adding ? t('@yovoice.voice.addTitle') : t('@yovoice.voice.pickTitle')}</h2><small>{t('@yovoice.voice.hint')}</small></VStack><Button label={adding ? t('@yovoice.voice.closeAdd') : t('@yovoice.voice.closePick')} className="voice-close" size="sm" isIconOnly icon={<X size={17} />} variant="ghost" isDisabled={recording || busy} onClick={onClose} /></HStack>
      {error ? <p className="dialog-error" role="alert">{error}</p> : null}
      <HStack gap={2} wrap="wrap"><Button label={t('@yovoice.voice.import')} size="sm" icon={<Upload size={16} />} isLoading={busy} isDisabled={recording} onClick={() => input.current?.click()} /><Button label={t('@yovoice.voice.record')} size="sm" variant="ghost" icon={<Mic size={16} />} isDisabled={busy || recording} aria-expanded={showRecording} onClick={() => { setShowRecording(value => !value); setCrop(null); }} /></HStack>
      ...
      {!adding ? <VStack className="voice-list" gap={0}>{voices.length ? voices.map(voice => <HStack key={voice.id} className="voice-row" gap={3} vAlign="center">
        ...
        <Button label={t('@yovoice.voice.crop')} size="sm" variant="ghost" isDisabled={recording || busy} onClick={() => { setCrop(voice); setShowRecording(false); setRange([0, voice.duration]); }} />
      </HStack>) : null}</VStack> : null}
      {preview ? <VStack gap={2}><small>{preview.name}</small><audio className="voice-preview" aria-label={t('@yovoice.voice.audition', { name: preview.name })} src={preview.url} controls autoPlay /></VStack> : null}
